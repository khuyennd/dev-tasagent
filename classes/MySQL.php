...
class MySQL extends Db
{
	public function connect()
	{
		...
		if ($this->_link = mysql_connect($this->_server, $this->_user, $this->_password))
		{
			...
			echo mysql_error();//print_r($this->_link);//die(Tools::displayError('Link to database cannot be established.'));
		/* UTF-8 support */
		if (!mysql_query('SET NAMES \'utf8\'', $this->_link))
			die(Tools::displayError('TAS Fatal error: no utf-8 support. Please check your server configuration.'));
	...
	{
		global $webservice_call;
		if (_TAS_DEBUG_SQL_ AND mysql_errno($this->_link) AND !defined('TAS_INSTALLATION_IN_PROGRESS'))
		{
			if ($query)
