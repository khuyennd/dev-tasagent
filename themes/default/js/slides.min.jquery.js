	...
	$.fn.slides=function(option){
		option=$.extend({},$.fn.slides.option,option);
	  	return this.each(function(){
			...
			control.css({position:'relative',width:160,height:160,left:-160});
			control.children().css({position:'absolute',top:0,left:160,zIndex:0,display:'none'});
			...
			if(option.preload&&control.children()[0].tagName=='IMG'){
				...
				control.children(':eq('+start+')').fadeIn(option.fadeSpeed,function(){loaded=true;});
			}
			if(option.bigTarget){
			...
			if(option.hoverPause&&option.play){
				control.children().bind('mouseover',function(){stop();});
				control.children().bind('mouseleave',function(){pause();});
			}
			...
			$('.'+option.paginationClass+' li',elem).addClass('hidden');
			
			$('.'+option.paginationClass+' li:eq('+start+')',elem).addClass('show current');
				...
				if(!active&&loaded){
					active=true;switch(direction){
							...
							if(option.crossfade){
								...
								control.children(':eq('+prev+')',elem).fadeOut(option.fadeSpeed,function(){
									if(option.autoHeight){
										...
										option.autoHeightSpeed,function(){control.children(':eq('+next+')',elem).fadeIn(option.fadeSpeed);});
									}
									else{
						...
						if(option.pagination){
							$('.'+option.paginationClass+' li.current',elem).addClass('hidden');
							$('.'+option.paginationClass+' li.current',elem).removeClass('show current');
							
							$('.'+option.paginationClass+' li:eq('+next+')',elem).removeClass('hidden');
							$('.'+option.paginationClass+' li:eq('+next+')',elem).addClass('show current');
						}
					}
				};
			});
